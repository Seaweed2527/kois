...
$q = mysqli_query($link, "SELECT dgw, tgw from moni where sn ='2021070002' order by no desc ");
...
$query = mysqli_query($link, "SELECT thd_ph1, thd_ph2, thd_tds1, thd_tds2, thd_ox1, thd_ox2, thd_sn1, thd_sn2 from plant_user where sn ='2021070002' order by no desc ");
...
//pH
$query3 = mysqli_query($link, "SELECT moni_detail.nilai from moni join moni_detail on moni.no =moni_detail.id where moni_detail.sensor='pH' and moni.sn='2021070002' ORDER BY moni_detail.id DESC limit 1");
$q4 = mysqli_fetch_row($query3);
$pH = $q4[0];
echo "pH saat ini : $pH <br>";

//TDS
$query4 = mysqli_query($link, "SELECT moni_detail.nilai from moni join moni_detail on moni.no =moni_detail.id where moni_detail.sensor='TDS' and moni.sn='2021070002' ORDER BY moni_detail.id DESC limit 1");
...
//Oxygen
//Water Level
$query5 = mysqli_query($link, "SELECT moni_detail.nilai from moni join moni_detail on moni.no =moni_detail.id where moni_detail.sensor='oksigen' and moni.sn='2021070002' ORDER BY moni_detail.id DESC limit 1");
$q6 = mysqli_fetch_row($query5);
$DO = $q6[0];
echo "Kadar oksigen saat ini : $DO<br>";

//suhu nutrisi
$query6 = mysqli_query($link, "SELECT moni_detail.nilai from moni join moni_detail on moni.no =moni_detail.id where moni_detail.sensor='reservoir_temp' and moni.sn='2021070002' ORDER BY moni_detail.id DESC limit 1");
...
//notif alat mati
if ($delay > 1) {
	$b = mysqli_query($link, "SELECT tgl, pkl, tipe from notif where sn ='2021070002' and hour(pkl)='$jam_end' and tgl = '$date' and tipe = 'alat' order by no desc limit 1");
	$bb = mysqli_fetch_row($b);

	// prep the bundle
	if (empty($bb)) {
		mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$e', 'alat', 'Alat mati yo!')");
		$msg = array(
		...
		$fields = array(
			'to'  => '/topics/systemkoi_hartono',
			'data'	=> $msg
		);
		$fieldsList[] = $fields;
	} else {
		$a = mysqli_query($link, "SELECT tgl, pkl, tipe FROM notif WHERE sn='2021070002' and tipe='alat' ORDER BY no DESC limit 1");
		...
		if ($delay_a >= 720) {
			mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$akhir', 'alat', 'Alat mati yo!')");
...
if ($pH > $thd_ph2 || $pH < $thd_ph1) {
	$c = mysqli_query($link, "SELECT tgl, pkl, tipe from notif where sn ='2021070002' and hour(pkl)='$jam_end' and tgl = '$date' and tipe = 'pH' order by no desc limit 1");
	$cc = mysqli_fetch_row($c);
	// prep the bundle
	if (empty($cc)) {
		mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$e', 'pH', 'pH is abnormal')");
		$msg = array(
		...
		$fields = array(
			'to'  => '/topics/pHkoi_hartono',
			'data'	=> $msg
		);
		$fieldsList[] = $fields;
	} else {
		$b = mysqli_query($link, "SELECT tgl, pkl, tipe FROM notif WHERE sn='2021070002' and tipe='pH' ORDER BY no DESC limit 1");
		...
		if ($delay_aa >= 720) {
			mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$akhir', 'pH', 'pH is abnormal!')");
			$msg = array(
				'message' 	=> 'Current pH : ' . $pH,
				'title'		=> 'Warning !!!',
				'vibrate'	=> 1,
...
if ($TDS > $thd_tds2 || $TDS < $thd_tds1) {
	$d = mysqli_query($link, "SELECT tgl, pkl, tipe from notif where sn ='2021070002' and hour(pkl)='$jam_end' and tgl = '$date' and tipe = 'TDS' order by no desc limit 1");
	$dd = mysqli_fetch_row($d);
	// prep the bundle
	if (empty($dd)) {
		mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$e', 'TDS', 'TDS is abnormal')");
		$msg = array(
		...
		$fields = array(
			'to'  => '/topics/TDSkoi_hartono',
			'data'	=> $msg
		);
		$fieldsList[] = $fields;
	} else {
		$c = mysqli_query($link, "SELECT tgl, pkl, tipe FROM notif WHERE sn='2021070002' and tipe='TDS' ORDER BY no DESC limit 1");
		...
		if ($delay_b >= 720) {
			mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$akhir', 'TDS', 'TDS is abnormal!')");
			$msg = array(
				'message' 	=> 'Current TDS :' . $TDS,
				'title'		=> 'Warning !!!',
				'vibrate'	=> 1,
...
if ($DO > $thd_ox2 || $DO < $thd_ox1) {
	$n = mysqli_query($link, "SELECT tgl, pkl, tipe from notif where sn ='2021070002' and hour(pkl)='$jam_end' and tgl = '$date' and tipe = 'oksigen' order by no desc limit 1");
	$nn = mysqli_fetch_row($n);
	// prep the bundle

	if (empty($nn)) {
		mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$e', 'oksigen', 'Oxygen Level is abnormal')");
		$msg = array(
		...
		if ($delay_x >= 720) {
			mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$akhir', 'oksigen', 'Oxygen level is abnormal!')");
			$msg = array(
				'message' 	=> 'Current oxygen level :' . $DO,
				'title'		=> 'Warning !!!',
				'vibrate'	=> 1,
...
if ($sunut > $thd_sn2 || $sunut < $thd_sn1) {
	$f = mysqli_query($link, "SELECT tgl, pkl, tipe from notif where sn ='2021070002' and hour(pkl)='$jam_end' and tgl = '$date' and tipe = 'Sn' order by no desc limit 1");
	$ff = mysqli_fetch_row($f);

	// prep the bundle
	if (empty($ff)) {
		mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$e', 'Sn', 'Water Temperature is abnormal')");
		$msg = array(
		...
		$fields = array(
			'to'  => '/topics/nutrisikoi_hartono',
			'data'	=> $msg
		);
		$fieldsList[] = $fields;
	} else {
		$z = mysqli_query($link, "SELECT tgl, pkl, tipe FROM notif WHERE sn='2021070002' and tipe='Sn' ORDER BY no DESC limit 1");
		...
		if ($delay_z >= 720) {
			mysqli_query($link, "insert into notif(sn, tgl, pkl, tipe, ket) values ('2021070002','$date', '$akhir', 'Sn', 'Water Temperature is abnormal'");
			// prep the bundle
			$msg = array(
